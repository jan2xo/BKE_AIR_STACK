        ...
        private sealed class Binding : IDisposable
        {
            ...
            internal Binding(Form owner)
            {
                ...
                _bell = new Button
                {
                    ...
                    AccessibleDescription = "Open Air Stack notifications",
                    Width = 52,
                    Height = 34,
                    Top = 12,
                    Left = Math.Max(12, owner.ClientSize.Width - 196),
            ...
            private async Task RefreshBadgeAsync()
            {
                ...
                try
                {
                    ...
                    _bell.Text = count.Count > 0 ? $"🔔 {count.Count}" : "🔔";
                    _toolTip.SetToolTip(
                        _bell,
                        count.Count > 0 ? $"Notifications — {count.Count} unread" : "Notifications");
                }
                catch (Exception ex)
                {
                    Debug.WriteLine($"Air Stack notification tray badge failed: {ex.Message}");
                }
                finally
                {
                    _refreshing = false;
            ...
            private async Task ShowTrayAsync()
            {
                ...
                async Task LoadAsync()
                {
                    ...
                    {
                        status.Text = "Notifications are temporarily unavailable.";
                        Debug.WriteLine($"Air Stack notification tray feed failed: {ex.Message}");
                    }
                    finally
                    {
            ...
            private Panel CreateItemPanel(Form tray, NotificationItem item, Func<Task> reload)
            {
                ...
                open.Click += async (_, __) =>
                {
                    ...
                    {
                        Debug.WriteLine($"Air Stack notification open failed: {ex.Message}");
                    }
                    finally
                    {
                        if (!tray.IsDisposed)
                ...
                dismiss.Click += async (_, __) =>
                {
                    ...
                    {
                        Debug.WriteLine($"Air Stack notification dismiss failed: {ex.Message}");
                    }
                    finally
                    {
                        if (!tray.IsDisposed)
            ...
            {
                Debug.WriteLine(
                    $"Air Stack notification tray {operation} failed: " +
