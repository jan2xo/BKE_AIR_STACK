...
param(
    ...
    [string]$ProductId = 'bke-air-stack',
    [string]$Version,
    [string]$EntryPoint = 'BKE AirStack.exe'
)
...
$payloadName = "Air-Stack-$Version-Windows-x64.update.zip"
$payloadPath = Join-Path $OutputDirectory $payloadName
$metadataPath = Join-Path $OutputDirectory "Air-Stack-$Version-Windows-x64.update.json"
